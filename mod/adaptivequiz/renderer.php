...
class mod_adaptivequiz_renderer extends plugin_renderer_base {
     ...
     * @param int $requireseb 0 = off, 1 = SEB presence only, 2 = SEB + key validation
     * @return string
     */
    public function attempt_controls_or_notification(int $cmid, bool $attemptallowed, bool $browsersecurityenabled, int $requireseb = 0): string {
        if (!$attemptallowed) {
            return html_writer::div(get_string('noattemptsallowed', 'adaptivequiz'), 'alert alert-info text-center');
        }

        if ($requireseb > 0) {
            // SEB only runs on Windows and macOS. Detect from User-Agent so students on
            // other platforms (Linux, Android, iOS) can still attempt the quiz directly.
            $ua = $_SERVER['HTTP_USER_AGENT'] ?? '';
            $sebsupported = (stripos($ua, 'Windows') !== false || stripos($ua, 'Macintosh') !== false);
            if ($sebsupported) {
                return $this->display_seb_launch_button($cmid);
            }
            // Unsupported OS — skip SEB enforcement and show the normal attempt button.
        }
    ...
    /**
     * Renders the "Launch Safe Exam Browser" button.
     *
     * The button uses a sebs:// (or seb://) link pointing to seb_config.php.
     * Clicking it tells the OS to open SEB, which fetches the config file from
     * that URL (substituting sebs→https), reads the plist, and navigates directly
     * to the adaptive quiz attempt page — no manual URL entry needed.
     *
     * @param int $cmid Course-module id.
     * @return string HTML.
     */
    private function display_seb_launch_button(int $cmid): string {
        $configurl = new moodle_url('/mod/adaptivequiz/seb_config.php', ['cmid' => $cmid]);
        $configurl->set_scheme(is_https() ? 'sebs' : 'seb');

        $html  = html_writer::div(
            get_string('sebrequired', 'adaptivequiz'),
            'alert alert-warning text-center mb-3'
        );
        $html .= html_writer::div(
            html_writer::link($configurl->out(false), get_string('seblaunchbtn', 'adaptivequiz'),
