...
class attempt_report_helper {
    ...
    /**
     * Calculates the weighted score for an attempt.
     *
     * Score formula: Σ(difficulty_level × correct_answers_at_that_level)
     * Total formula: Σ(difficulty_level × all_answers_at_that_level)
     *
     * @param int $attemptid
     * @return array Keys: 'score' (int), 'total' (int), 'percent' (float)
     */
    public static function calculate_attempt_score(int $attemptid): array {
        $distribution = self::prepare_answers_distribution_data($attemptid);

        $score = 0;
        $total = 0;
        foreach ($distribution as $level => $data) {
            $l = (int) $level;
            $c = (int) $data->numcorrect;
            $w = (int) $data->numwrong;
            $score += $l * $c;
            $total += $l * ($c + $w);
        }
